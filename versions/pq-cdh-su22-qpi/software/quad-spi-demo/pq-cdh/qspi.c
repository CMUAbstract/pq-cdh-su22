...
void init_qspi(){
  ...
  */
  ...
  // qspi_writeInstruction(0x00);
  // qspi_writeInstruction(0xff);
  
  uint8_t test_buffer[4] = {0x0f, 0x00, 0x00, 0x00};
  uint32_t len = 1;
  uint32_t test_address = 0x03;
  qspi_writeData(test_buffer, len, test_address);
  qspi_readData(test_buffer, len, test_address);
